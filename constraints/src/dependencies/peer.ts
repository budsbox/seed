...
export const constraintPeerDependencies: Constraint = ({ Yarn }): void => {
  ...
  const peersMap = new Map<string, readonly YarnType.Constraints.Dependency[]>(
    Yarn.workspaces().flatMap((workspace) => {
      ...
      return workspace.ident != null ?
          [
            [
              workspace.ident,
              Yarn.dependencies({ workspace, type: 'peerDependencies' }).filter(
