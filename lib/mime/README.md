...
Built on top of a PEG.js-based parser (`@budsbox/parse-mime`) and the community-maintained [`mime-db`](https://github.com/jshttp/mime-db) database, the library is split into two entry points so you can keep your bundle lean when you only need core parsing and manipulation.
...
-   **40+ guard functions** — `isJson`, `isImage`, `isArchive`, `isCompressible`, `isVendorTree`, and many more
...
| `@budsbox/lib-mime/meta` | Metadata queries, canonicalization, guards, sniffing | Yes                |
...
update('application/json', 'type', 'text'); // 'text/json'
